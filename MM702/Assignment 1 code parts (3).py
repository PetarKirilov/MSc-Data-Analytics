...
#remove outliers, sitting 4 standard deviations away from the mean
#abalone = abalone[np.abs(abalone["Rings"]-abalone["Rings"].mean())<=(4*abalone["Rings"].std())]
...
#abalone["RingsAge"] = pd.cut(abalone["Rings"], [0,8,14,29])
#normalize the data
...
x_train, x_test, y_train, y_test = train_test_split(abalone_data, abalone_target, random_state = 15, test_size = 0.2)
...
steps = [("scaler", StandardScaler()), ("knn", KNeighborsClassifier())]
pipeline = Pipeline(steps)
x_train, x_test, y_train, y_test = train_test_split(abalone_data, abalone_target)
knn_scaled = pipeline.fit(x_train, y_train)
print(knn_scaled.score(x_test, y_test))
knn_unscaled = KNeighborsClassifier().fit(x_train, y_train)
print(knn_unscaled.score(x_test, y_test))
...
#adult_target = adult_target.reshape(-1,1)
scaler = preprocessing.StandardScaler()
...
#sample = encoded_data[["marital-status", "education-num", "relationship", "age"]].values
#varieties = encoded_data["gross_income"].values
