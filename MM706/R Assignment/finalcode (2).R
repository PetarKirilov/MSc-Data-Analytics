...
table1 <- nas[,c(13,15)]          #add the Title and Gender column to a table to look at the proportion of Males/Females
table1$Gender[is.na(table1$Gender)] <- 0 #assign 0 to NA values to be visible in prop.table function
round(prop.table(table(table1), 2)*100)      #from the table it is evident that the majority (90.7%) with title "Sig" are males, 
