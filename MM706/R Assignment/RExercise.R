...
ttt$Gender <- NULL #an example on how to remove a column
ttt$Gender <- NA #to add an empty column
ttt$Gender[ttt$Title == "Mr"] <- "Male" #adds Male to the gender column
ttt$Gender[ttt$Title == "Ms" | ttt$Title == "Mrs" | ttt$Title == "Miss"] <- "Female" #adds female to the gender column

nas <- ttt[is.na(ttt$Gender),] #add the NA's to a new DF
library(gender)
